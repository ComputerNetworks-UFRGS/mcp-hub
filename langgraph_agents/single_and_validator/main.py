...
async def build_graph():
    def validator_condition(state):
        ...
        if not last_message.content or 'GOOD ANSWER' in last_message.content:
            return END
        else:
            return "try_again"
    ...
    validator_agent = ValidatorAgent(llm=model)
    ...
    graph.add_conditional_edges(
        "single_agent",
        tools_condition,
        {"tools": "tool_node", END: "validator_agent"})
    graph.add_edge("tool_node", "single_agent")

    graph.add_conditional_edges(
        "validator_agent",
        validator_condition,
        {"try_again": "single_agent", END: END})
...
async def main():
    ...
    while True:
        ...
        final_state = app.get_state(config=config).values# type: ignore
